...
async def test_reset_args(
    ...
    async def fsm_handler(message: IncomingMessage, _: Bot) -> None:
        await message.state.fsm.change_state(
            EnumForTests.FIRST_STATE, message="Hello Friend!"
        )
    ...
    # - Assert -
    assert first_message.state.fsm_storage.message == "Hello World!"
    assert second_message.state.fsm_storage.message == "Hello World!"


async def test_augment_args(
    bot_account: BotAccountWithSecret,
    incoming_message_factory: Callable[..., IncomingMessage],
) -> None:
    # - Arrange -
